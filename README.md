...
2. Credits are checked
3. If credits < threshold → command is immediately discarded and state restored
4. No retry - it moves on to the next command
So commands are "fail-fast" - they're dropped immediately when credits are insufficient, not held for later retry.
...
| `COMMAND_TIMEOUT` | `command_timeout` | `1m` | Max time a command waits in queue before being discarded |
