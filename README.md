...
-   **TX Handshaking**:
    ...
    -   Blocks concurrent commands to the same device while waiting for ACK.
...
2. Check if device already has a pending TX → if yes, reject and restore state
3. Credits are checked via `X` command
4. If credits < threshold or queue busy → command is immediately discarded and state restored
5. Command is sent to CUL stick
6. **LOVF check**: If CUL returns "LOVF" → abort TX and restore state
7. **Wait for ACK**: Wait for thermostat to respond (Type `0x02`) up to `command_timeout`
8. If timeout → restore state; if ACK received → success!
9. No retry - it moves on to the next command
...
sequenceDiagram
    ...
    alt LOVF Response (TX buffer overflow)
        ...
        DM->>DM: Mark device as pending TX
        loop Wait for ACK (up to command_timeout)
            ...
            alt Matching ACK from target device
                DM->>DM: Clear pending TX - Success!
            else RX from other device
                DM->>DM: Process normally (not blocked)
            end
        end
...
- **Per-Device Blocking**: While waiting for ACK from device A, new commands to device A are rejected. Commands to device B proceed normally.
- **Other RX Processing**: RX packets from other thermostats are processed normally during the wait.
