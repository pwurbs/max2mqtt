...
4.  **Priority**: If a user manually changes the temperature on the device immediately after an MQTT command, the verification logic sees a mismatch and will resend the MQTT command to enforce the requested value (as the verification target is always the last sent command).
