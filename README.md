...
Never adapt temperature too quickly due to the Ack handshake. If there is no Ack packet from the former action, the new command will be rejected.
So, don't use the +/- buttons in a HA card when you want to change more than 0.5°C. For this, better use the wheel on the climate card.
...
| `COMMAND_TIMEOUT` | `command_timeout` | `2m` | **Dual purpose**: (1) Max time a command waits in queue before being discarded, and (2) Timeout for waiting for thermostat ACK after TX |
